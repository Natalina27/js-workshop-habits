...
3. Выполни в консоли **`npm -v`** и убедись, что установленная версия **`npm`** не ниже **`6.14.5`**;

> ❗️ Мы поддерживаем только последние **`LTS-версии Node.js`** (текущая LTS — **`Node 12.18.3`**). Мы не даём гарантий работы на других версиях Node. Если у тебя не работает, в первую очередь проверь версию Node.js!
...
```bash
git clone https://github.com/Lectrum/js-workshop-habits.git
```

7. Перейди в проект, выполнив команду:

```bash
cd js-workshop-habits
```
